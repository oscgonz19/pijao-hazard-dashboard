"""
Pagina 2: Mapa Interactivo - Visualizacion de amenaza
...
# SIDEBAR
# ==============================================================================
st.sidebar.title("Configuracion del Mapa")
...
# CONTENIDO PRINCIPAL
# ==============================================================================
st.title("🗺️ Mapa de Amenaza Geotecnica")
...
if data['raster_overlay'] and data['raster_overlay'].get('reclassified'):
    st.warning(
        "El raster contiene valores continuos (FS). "
...
if puntos_filtrados is not None and len(puntos_filtrados) > 0:
    ...
    top_puntos['Accion'] = top_puntos['haz_num'].apply(
        lambda x: 'Inmediata' if x >= 4 else ('Monitoreo' if x == 3 else 'Rutina')
    )
